          ...
          <div class="flex flex-col flex-grow">
            ...
            <div class="relative w-full flex-grow bg-gray-50 p-2">
              ...
              <div class="flex" v-if="store.provider">
                <div class="w-10/12 mx-auto bg-white rounded p-5">
                  <div class="flex">
                    <div class="w-2/12">
                      <img v-if="store.provider.displayLogo" :src="store.provider.displayLogo" style=" height: 65px;" class="mx-auto" alt="thumb" />
                      <i v-else class="text-center fa-solid fa-bank rounded-full bg-gray-500 p-5 text-white" style="font-size: 40px;"></i>
                    </div>
                    <div class="w-10/12 my-auto">
                      <p class="font-bold text-xl capitalize">{{store.provider.name}}</p>
                      <p class="text-sm capitalize">{{store.provider.displayName}}</p>
                    </div>
                  </div>
                  <div class="flex">
                    <div class="w-11/12 mx-auto py-5 text-xs">
                      <table>
                        <tbody>
                        <tr>
                          <td class="font-bold px-5">Email</td>
                          <td class="px-5">{{store.provider.inquiryEmail}}</td>
                        </tr>
                        <tr>
                          <td class="font-bold px-5">Phone</td>
                          <td class="px-5">{{store.provider.inquiryPhoneNumber}}</td>
                        </tr>
                        <tr>
                          <td class="font-bold px-5">Onboarded On</td>
                  ...
                  <div class="flex pt-5">
                    ...
                    <div :class="(activeTab == 'callbacks') ? 'w-2/12 tab-active' :'w-2/12 tab'" @click="select('callbacks')">Callbacks/Webhooks</div>
