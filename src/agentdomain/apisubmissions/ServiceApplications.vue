      ...
      <div class="w-9/12">
        <div class="flex bg-gray-10 border border-gray-200 rounded px-2 py-3">
          ...
          <select class="filter-element w-3/12 e-select" v-model="serviceId" @change="fetch">
            ...
            <option v-for="(service, idx) in serviceStore.services" :key="idx" :value="service.id">{{service.name}}</option>
          </select>
    ...
    <div class="flex my-1">
      <table class="table">
        ...
        <tr class="body-tr" v-for="(application, idx) in store.applications" :key="idx">
          ...
          <td>
            <div v-if="application.status === 'PENDING'"  class="pill-inert text-center">
              <p>{{ application.status }}</p>
            </div>
            <div v-if="application.status === 'AWAITING_PAYMENT'"  class="pill-idle text-center">
              <p>{{ application.status.replace("_", " ") }}</p>
            </div>
            <div v-if="application.status === 'SENT'"  class="pill-semi-active text-center">
              <p>SUBMITTED</p>
            </div>
            <div v-if="application.status === 'APPROVED'" class="pill-active text-center">
              <p>{{ application.status }}</p>
            </div>
            <div v-if="application.status === 'QUERIED'"  class="pill-warning text-center">
              <p>{{ application.status }}</p>
            </div>
            <div v-if="application.status === 'REJECTED' || application.status === 'PAYMENT_FAILED'"  class="pill-danger text-center">
              <p>{{ application.status }}</p>
            </div>
          </td>
    ...
    <div class="flex">
      <div class="w-full">
        <div class="flex" v-if="limit == store.applications?.length || page > 1">
          <button v-if="page > 1" class="pagination-button" @click="previous"> <i class="fa-solid fa-arrow-left"></i> Previous </button>
          <button v-else class="pagination-button-inert" @click="next"><i class="fa-solid fa-arrow-left"></i> Previous</button>
          <div class="w-1/12 text-center my-auto">
            <label class="rounded text-white bg-primary-700 px-3 py-1">{{page}}</label>
          </div>
          <button v-if="limit == store.applications?.length" class="pagination-button" @click="next">Next <i class="fa-solid fa-arrow-right"></i></button>
          <button v-else class="pagination-button-inert" @click="next">Next <i class="fa-solid fa-arrow-right"></i></button>
        </div>
      </div>
    </div>
  </div>
